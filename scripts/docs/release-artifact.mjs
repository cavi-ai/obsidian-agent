...
import { resolveReleaseIdentity } from "./lib.mjs";
...
async function collectFiles(root, archiveRoot) {
  ...
  async function visit(directory, relativeDirectory = "") {
    ...
    for (const entry of entries) {
      ...
      else if (info.isFile()) files.push({ name: `${archiveRoot}/${relativePath}`, contents: await readFile(absolute) });
...
export async function createProductDocsReleaseArtifact(input) {
  ...
  if (input.repository !== "cavi-ai/obsidian-agent") throw new Error("invalid release repository");
  const manifest = {
    schemaVersion: 1,
    slug: "obsidian-agent",
    kind: "product-docs",
  ...
  const files = await collectFiles(
    input.docsRoot,
    `docs/obsidian-agent/v${release.version}`,
  );
  ...
  const tar = Buffer.concat([...entries.map((entry) => tarEntry({ ...entry, epoch: release.sourceDateEpoch })), Buffer.alloc(BLOCK * 2)]);
  const archive = gzip(tar, release.sourceDateEpoch);
  const artifactSha256 = sha256(archive);
  const artifactName = `obsidian-agent-docs-${release.tag}.tar.gz`;
