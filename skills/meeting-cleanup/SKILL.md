...
1. **Read the raw note** with
   `obsidian read vault=<vault> file=<raw-path>`.
...
4. **Verify prospective links.** Run
   `obsidian search vault=<vault> query=<mention> format=json` for mentioned
...
5. **Preview and write.** Show the complete Markdown and either the full
   ...
   `obsidian create vault=<vault> path=<path> content=<markdown> overwrite` for
